...
v_A=0.003; v_B=0.04; v_C=0.04; v_D=0.04; % advecção vertical
...
% definindo a posição de cada espécie
x_center = nnx / 2;
y_center = nny / 2;
radius = 2;

% espécie A, distribuída no primeiro quadrante do domínio
for i=1:nnx
  for j=1:nny
    ind=(i-1)*nny+j;
    if (i <= x_center && j <= y_center && (i-x_center)^2 + (j-y_center)^2 < radius^2)
        Az(ind)=K/7;
    end
    
    % espécie B, distribuída no segundo quadrante do domínio
    if (i <= x_center && j > y_center && (i-x_center)^2 + (j-y_center)^2 < radius^2)
        Bz(ind)=K/7;
    end
    
    % espécie C, distribuída no terceiro quadrante do domínio
    if (i > x_center && j <= y_center && (i-x_center)^2 + (j-y_center)^2 < radius^2)
        Cz(ind)=K/7;
    end
    
    % espécie D, distribuída no quarto quadrante do domínio
    if (i > x_center && j > y_center && (i-x_center)^2 + (j-y_center)^2 < radius^2)
        Dz(ind)=K/7;
    end
  end
...
%%%%%%%%%%%%%%%%%%%
for k=1:nn
nDe(k,k)=De(k,k)+(lambda_D/(K))*((gama_D*Aint(k)+sigma_D*Bint(k)+delta_D*Cint(k)+Dint(k)))*dt/2;
