...
% Espécie a: GOIABA DE ANTA, vermelha
% Espécie b: LACRE, preta
% Espécie c: UCUUBA, Azul
% Espécie d: ANDIROBA, verde
alfa_A=0.025; alfa_B=0.035; alfa_C=0.037; alfa_D=0.03; %difusão
u_A=0.002; u_B=0.001; u_C=0.01;u_D=0.01; % advecção horizontal
v_A=0.04; v_B=0.04; v_C=0.03; v_D=0.03; % advecção vertical
mu_A=0.0; mu_B=0.0; mu_C=0.0; mu_D=0.0; % decaimentos - de início considerados nulos
lambda_A=0.7; lambda_B=0.5; lambda_C=0.7; lambda_D=0.6;  % taxas de crescimento intr¨ªnseco
gama_A=0.005; gama_B=0.05; gama_C=0.04; gama_D=0.03;% competição interespecíficas
sigma_A=0.029; sigma_B=0.16; sigma_C=0.06; sigma_D=0.01; % competição interespecíficas
delta_A=0.5; delta_B=0.6; delta_C=0.07; delta_D=0.08; % competição interespecíficas
...
for i=1:nnx
  for j=1:nny
    ...
    if mod(i+j,4) == 1 % espécie A
        Az(ind)=K/4;
    elseif mod(i+j,4) == 2 % espécie B
        Bz(ind)=K/4;
    elseif mod(i+j,4) == 3 % espécie C
        Cz(ind)=K/4;
    else % espécie D
        Dz(ind)=K/4;
    end
...
%figure(6)
 %hold on
 %subplot(3,2,1), surf(verA)
 %axis([0 6 0 8 0 30])
 %subplot(3,2,2), surf(verB)
 %axis([0 6 0 8 0 30])
 %subplot(3,2,3), surf(verC)
 %axis([0 6 0 8 0 30])
 %subplot(3,2,4), surf(verD)
 %%axis([0 6 0 8 0 30])
...
%[X,Y] = meshgrid(1:nnx, 1:nny);
%Z = reshape(Az, [nny nnx]);
%figure(11);
%surf(X,Y,Z);
%[X,Y] = meshgrid(1:nnx, 1:nny);
%Z = reshape(Bz, [nny nnx]);
%figure(12);
%surf(X,Y,Z);
%[X,Y] = meshgrid(1:nnx, 1:nny);
%Z = reshape(Cz, [nny nnx]);
%figure(13);
%surf(X,Y,Z);
%[X,Y] = meshgrid(1:nnx, 1:nny);
%Z = reshape(Dz, [nny nnx]);
%figure(14);
%surf(X,Y,Z);
figure(1);
...
%figure(6)
%hold on
%subplot(3,2,1), cla, surf(verA)
%xlabel("LENGTH")
%ylabel("WIDTH")
%zlabel("POPULATION DENSITY")
%axis([0 40 0 30 0 20])
%subplot(3,2,2), cla, surf(verB)
%xlabel("LENGTH")
%ylabel("WIDTH")
%zlabel("POPULATION DENSITY")
%axis([0 40 0 30 0 20])
%subplot(3,2,3), cla, surf(verC)
%xlabel("LENGTH")
%ylabel("WIDTH")
%zlabel("POPULATION DENSITY")
%axis([0 40 0 30 0 20])
%subplot(3,2,4), cla, surf(verD)
%xlabel("LENGTH")
%ylabel("WIDTH")
%zlabel("POPULATION DENSITY")
%axis([0 40 0 30 0 20])
